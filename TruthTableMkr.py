...
if int(c)==0:
    try:
        ...
        s=input("Please close the file first and then type \"save\": ")
        while s.lower()!='save':
            s=input("Please close the file first and then type \"save\": ")
        book.save("result.xlsx")
    f=input("Your truth table has been made. You may open the file \"result.xlsx\" and type your outputs. Make sure you save and close the file after you are done. Please type \"done\" if you are done: ")
elif int(c)==16:
    ...
    while y!='done' and count!=2**(int(a)):
        if y!='done':
            try:
                ...
                print("Please type single characters per line")
                y=input()
                x=seg1dict[y]
    ...
    try:
        ...
        while s.lower()!='save':
            s=input("Please close the file first and then type \"save\": ")
        book.save("result.xlsx")
    print("Your truth table and equations have been generated")
elif int(c)==7:
    ...
    while count<2**(int(a)):
        for t in range(0,int(c)):
            sheet[arr[t+int(a)+1]+str(3+count)]='x'
        count+=1
    try:
        book.save("result.xlsx")
    except PermissionError:
...
for y in range(0,int(b)):
    ...
    for b in range(0,len(z)):
        for c in range(0,len(z[b])):
            if z[b][c]=='0':
                simp+=('!'+arr2[c]+' & ')
            elif z[b][c]=='1':
                simp+=((arr2[c])+' & ')
        simp=simp[:len(simp)-2]
        simp+='# '
    simp=simp[:len(simp)-3]
    if len(simp)==0:
        simp+='1'
    simp+='\n'
    st='y'+str(y)+' = '+simp+' ;'
